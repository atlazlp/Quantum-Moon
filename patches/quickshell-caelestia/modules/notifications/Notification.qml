...
StyledRect {
    ...
    readonly property int notifStripW: Math.round(Tokens.sizes.notifs.width * LayoutTweaks.rightStripWidthScale)
    readonly property int nonAnimHeight: summary.implicitHeight + (root.expanded ? appName.height + body.height + actions.height + actions.anchors.topMargin : bodyPreview.height) + inner.anchors.margins * 2
    property bool expanded: Config.notifs.openExpanded

    function dismissPopup(): void {
        if (root.modelData.popup)
    ...
    MouseArea {
        ...
        onReleased: event => {
            ...
            if (Math.abs(root.x) < notifStripW * Config.notifs.clearThreshold)
                root.x = 0;
            else
                root.modelData.popup = false;
        }
        ...
        onClicked: event => {
            if (event.button !== Qt.LeftButton || GlobalConfig.notifs.actionOnClick === false)
                return;

            const local = expandBtn.mapFromItem(this, event.x, event.y);
            if (local.x >= 0 && local.x <= expandBtn.width && local.y >= 0 && local.y <= expandBtn.height)
                return;

            root.invokePrimaryAction();
        }
